...
int main(int argc, char* argv[])
{
    ...
    else if (task == 1)
    {
        try
        {
            shabalin::matrixInput(input, tempMatrix, countOfElements);
            output << shabalin::findLargeSeriesOfEqualElements(tempMatrix, rows, cols) << '\n';
            if (!output)
            {
                throw std::logic_error("Error of output");
            }
            smoothedMatrix = shabalin::makeUpSmoothMatrix(tempMatrix, tempSmoothedMatrix, rows, cols);
            shabalin::matrixOutput(output, tempSmoothedMatrix, rows, cols);
        }
        catch (const std::exception& e)
        {
            std::cerr << e.what() << '\n';
